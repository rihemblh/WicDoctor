...
apiurl="http://localhost:3001/getclinics"
 ...
 .then(response => {
     if (!response.ok) {
         throw new Error('Erreur lors de la récupération des données des clinic');
     }
     return response.json();
 })
 .then(data => {
     console.log('clinics récupérées:', data);
     displayClinicsInCards(data);

 })
 .catch(error => {
     console.error('Erreur:', error);
      const container = document.getElementById('clinic');
     container.innerHTML = '<center><h2> Pas de clinique trouvé</h2></center>'; 
 });
 function displayClinicsInCards(clinics) {
    ...
    if (!clinics || clinics.length === 0) {
        ...
        clinics.forEach(item => {
            ...
            const photosC = item.clinic_photos || "clinic.jpg";
            console.log("photos: ",photosC)
            const cardHTML = `
                        ...
                        <div class="item-card9-img" style="width: 122px;height: 106px;border-radius: 50%;object-fit: cover;">
                            <div class="item-card9-imgs">			
                                <img alt="img" class="cover-image"
                                    src="assets/images/media/cliniques/${photosC}">
