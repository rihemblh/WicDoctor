...
apiurl = `http://localhost:3001/getspecialitiesparclinics/19}`
console.log(`http://localhost:3001/getspecialitiesparclinics/${Detailsclinic.clinic_id}`)
...
function getSelectedSpeciality() {
	...
	if (selectedSpeciality) {
		...
		console.log(`http://localhost:3001/patternsclinic/${Detailsclinic.clinic_id}/${selectedSpeciality.value}`)
		fetch(`http://localhost:3001/patternsclinic/${Detailsclinic.clinic_id}/${selectedSpeciality.value}`)
			.then(response => {
				if (!response.ok) {
...
function getSelectedPatern() {
	...
	if (selectedPatern) {
		...
		console.log("Selected Pattern:", selectedPatern.value, selectedPatern, `http://localhost:3001/doctorsspeciality/${Speciality}/${Detailsclinic.clinic_id}/${selectedPatern.value}`); // Log the selected value
		fetch(`http://localhost:3001/doctorsspeciality/${Speciality}/${Detailsclinic.clinic_id}/${selectedPatern.value}`)
			.then(response => {
				if (!response.ok) {
...
function selectDoctor(index) {
	...
	console.log(`http://localhost:3001/availability/${Detailsclinic.clinic_id}/${selectedDoctor.doctor_id}`)
	fetch(`http://localhost:3001/availability/${Detailsclinic.clinic_id}/${selectedDoctor.doctor_id}`)
		.then(response => {
			if (!response.ok) {
